...
画笔松开后按固定参数依次执行距离过滤、抽稀和平滑，持久化处理后的点，且不能阻塞下一次输入。方框、箭头和画笔的最小有效几何阈值由当前线宽确定；不满足阈值的操作不产生命令、元素或 revision。
...
- `points` 保存确定性距离过滤、抽稀和平滑后的物理像素点，至少包含首点和尾点；相同输入和参数必须得到相同结果。
...
- 画笔处理结果具有确定性；方框、箭头、画笔低于线宽派生阈值时不创建元素或 revision。
