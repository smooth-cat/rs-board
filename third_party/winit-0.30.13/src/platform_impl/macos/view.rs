...
enum InsertTextCommitKind {
    MarkedText,
    Async,
}

fn has_disallowed_control_character(string: &str) -> bool {
    string.chars().any(|character| character.is_control() && !matches!(character, '\n' | '\r'))
}

fn insert_text_commit_kind(
    string: &str,
    has_marked_text: bool,
    ime_allowed: bool,
    view_focused: bool,
    interpreting_key_events: bool,
) -> Option<InsertTextCommitKind> {
    if string.is_empty()
        || has_disallowed_control_character(string)
        || !ime_allowed
        || !view_focused
    {
        return None;
...
fn selected_range_for_state(ime_allowed: bool, view_focused: bool) -> NSRange {
    if ime_allowed && view_focused {
        NSRange::new(0, 0)
    } else {
...
fn is_dictation_command(command: Sel) -> bool {
    command == sel!(startDictation:) || command == sel!(stopDictation:)
}
...
struct InterpretingKeyEventsGuard<'a> {
    interpreting_key_events: &'a Cell<bool>,
    previous_value: bool,
}

impl<'a> InterpretingKeyEventsGuard<'a> {
    fn new(interpreting_key_events: &'a Cell<bool>) -> Self {
        let previous_value = interpreting_key_events.get();
        interpreting_key_events.set(true);
        Self { interpreting_key_events, previous_value }
    }
}

impl Drop for InterpretingKeyEventsGuard<'_> {
    fn drop(&mut self) {
        self.interpreting_key_events.set(self.previous_value);
    }
}
...
declare_class!(
    ...
    unsafe impl NSTextInputClient for WinitView {
        ...
        fn selected_range(&self) -> NSRange {
            ...
            selected_range_for_state(self.ivars().ime_allowed.get(), self.is_first_responder())
        }
        ...
        fn first_rect_for_character_range(
            ...
            if !actual_range.is_null() {
                // Winit does not expose surrounding text, so its reported document is the
                // zero-length insertion point returned by `selectedRange`.
                unsafe { *actual_range = NSRange::new(0, 0) };
            }
            let rect = NSRect::new(
        ...
        #[method(windowLevel)]
        fn window_level(&self) -> NSInteger {
            trace_scope!("windowLevel");
            unsafe { self.window().level() }
        }
        ...
        fn do_command_by_selector(&self, command: Sel) {
            ...
            if is_dictation_command(command) {
                ...
                if should_forward_dictation_command(command, is_repeat) {
                    ...
                    let _: () = unsafe { msg_send![super(self), doCommandBySelector: command] };
                }
                return;
            }
            ...
            if self.ivars().ime_allowed.get() {
                let events_for_nsview = NSArray::from_slice(&[&*event]);
                {
                    let _interpreting_key_events =
                        InterpretingKeyEventsGuard::new(&self.ivars().interpreting_key_events);
...
impl WinitView {
    ...
    fn is_first_responder(&self) -> bool {
        self.window().firstResponder().is_some_and(|first_responder| *first_responder == ***self)
    }
...
mod tests {
    ...
    use super::{insert_text_commit_kind, InsertTextCommitKind};

    #[test]
    fn interpreting_key_events_guard_restores_previous_state() {
        let interpreting_key_events = Cell::new(false);
        {
            let _guard = super::InterpretingKeyEventsGuard::new(&interpreting_key_events);
            assert!(interpreting_key_events.get());
        }
        assert!(!interpreting_key_events.get());
    ...
    #[test]
    fn insert_text_commits_marked_text_while_interpreting_key_event() {
        assert_eq!(
            insert_text_commit_kind("候选\n", true, true, true, true),
            Some(InsertTextCommitKind::MarkedText)
        );
    }

    #[test]
    fn insert_text_ignores_regular_keyboard_input_during_key_interpretation() {
        assert_eq!(insert_text_commit_kind("a", false, true, true, true), None);
    }

    #[test]
    fn insert_text_commits_async_system_text_when_focused_and_ime_allowed() {
        assert_eq!(
            insert_text_commit_kind("dictated text\nnext line", false, true, true, false),
            Some(InsertTextCommitKind::Async)
        );
    }
    ...
    fn insert_text_filters_unavailable_or_control_input() {
        for (text, ime_allowed, focused) in [
            ...
            ("dictated", true, false),
            ("bad\ttext", true, true),
            ("bad\u{7f}text", true, true),
        ] {
            assert_eq!(insert_text_commit_kind(text, false, ime_allowed, focused, false), None);
        }
    }

    #[test]
    fn insert_text_allows_cr_and_lf_from_async_system_text() {
        for text in ["\n", "\r", "\r\n", "first\r\nsecond"] {
            assert_eq!(
                insert_text_commit_kind(text, false, true, true, false),
                Some(InsertTextCommitKind::Async)
            );
        }
    }
    ...
    #[test]
    fn selected_range_reports_insertion_point_only_for_focused_ime_input() {
        let focused_ime_range = super::selected_range_for_state(true, true);
        assert_eq!(focused_ime_range.location, 0);
        assert_eq!(focused_ime_range.length, 0);

        for (ime_allowed, focused) in [(false, true), (true, false), (false, false)] {
            let range = super::selected_range_for_state(ime_allowed, focused);
            assert_eq!(range.location, super::NSNotFound as super::NSUInteger);
            assert_eq!(range.length, 0);
        }
    }

    #[test]
    fn only_system_dictation_actions_are_forwarded_to_appkit() {
        assert!(super::should_forward_dictation_command(sel!(startDictation:), false));
        assert!(super::should_forward_dictation_command(sel!(stopDictation:), false));
        assert!(!super::should_forward_dictation_command(sel!(startDictation:), true));
